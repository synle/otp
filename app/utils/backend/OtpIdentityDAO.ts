...
export async function updateOtpIdentity(email: string, id: string, body: Partial<OtpIdentity> & {name: string}){
  ...
  fs.writeFileSync(_getOtpIdentityFilePath(email), JSON.stringify(otpIdentityResponse, null, 2))
}
